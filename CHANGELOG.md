...
- 순서 이동 아이콘을 굵고 둥근 상·하 분리 화살표로 변경
- 스캔 카드의 개별 회전 버튼을 제거하고 미리보기 안으로 이동
- 스캔 페이지 `전체 회전` 기능 추가
- 전체 회전 시 모든 페이지 OCR 캐시를 무효화하고 미리보기를 갱신

## v1.1.10 — 2026-09-16

- Drag 중 카드 테두리를 검은색 계열로 바꾸고, 삽입 위치 표시선은 투명 처리해 이동 자체만 자연스럽게 보이도록 조정.
- 스캔 페이지 표기를 `[번호] 페이지`에서 `현재/전체 페이지` 형식으로 변경하되 현재 번호 직접 입력 이동은 유지.
- 스캔 카드에서 컬러·흑백·문서·영역 조정·회전을 한 줄로 압축하고 삭제 버튼을 우측 상단으로 이동.
- `≡` 순서 손잡이를 깔끔한 위아래 화살표 `↕`로 변경.
- PDF 편집 페이지 카드도 스캔 카드와 같은 썸네일/헤더/삭제/페이지 번호 리듬으로 통일.
- PDF 편집의 전체 선택·선택 추출·OCR 수정·선택 삭제·전체 비우기 도구를 한 줄 compact toolbar로 정리.


## v1.1.9 — 2026-09-16

- 스캔 페이지 카드를 세로형 대형 미리보기에서 **작은 전체 문서 썸네일 + 오른쪽 조작부**의 가로형 compact card로 변경.
- 썸네일은 `object-fit: contain`으로 문서 전체가 보이도록 하고, 누르면 기존 전체화면 미리보기를 그대로 사용.
- 스캔/PDF 편집 Drag & Drop에 잡은 카드의 떠오름 효과, 파란 삽입 위치선, `여기에 놓기` 표시를 추가해 놓일 위치를 명확하게 표시.
- Drag 중 화면 상·하단 자동 스크롤과 전용 손잡이(`≡`) 조작을 유지.
- v1.1.8의 빈 작업 영역 숨김, PDF → 이미지 미리보기/회전, PDF 편집 기능을 그대로 유지.
- 앱/Service Worker/보안 런타임 캐시 버전을 1.1.9로 갱신.
