# ScanBox v1.1.3 Security Notes

## 문서 보정 엔진의 장애 격리

v1.1.3부터 OpenCV는 품질/속도를 높이는 가속 엔진으로 취급합니다. OpenCV 실행 또는 WASM 초기화가 실패해도 스캔 파일 자체를 외부로 보내지 않으며, 앱에 포함된 JavaScript 기반 문서 감지·원근 보정 경로로 자동 전환합니다. 따라서 외부 엔진의 가용성이 수동 4점 보정이나 핵심 스캔 기능을 차단하지 않습니다.
